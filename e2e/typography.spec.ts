...
test('high density gets the fine cut back @type', async ({ browser }) => {
  ...
  try {
    ...
    const notFine = roles.filter(
      ({ opsz }) => !Number.isFinite(opsz) || opsz < 14,
    );
    expect(
      notFine,
      "at 2dppx the fine cut must apply: no Bodoni role below 'opsz' 14",
    ).toEqual([]);
    ...
    const tooFine = roles.filter(({ opsz }) => opsz > 22);
    expect(tooFine, "the fine cut stops at 'opsz' 22").toEqual([]);
  } finally {
    await context.close();
  }
});
